...
- **Face detection** — [MediaPipe Face Landmarker](https://ai.google.dev/edge/mediapipe/solutions/vision/face_landmarker)
  ...
  on-device (WASM/WebGL), and no video ever leaves the browser.
...
- **Pupil physics** — each pupil is a ball simulated in screen space inside a
  ...
  - gravity pulls the pupil to the bottom of the socket; it scales with eye
    size, so every eye wobbles at the same tempo (~4 Hz, like the real toy);
